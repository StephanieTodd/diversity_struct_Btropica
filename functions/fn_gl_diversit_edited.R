# mappinggl order Davies Creek Emu Creek Spurgeon Tinaroo

# population colours for plotting 
#popcol4mappingl <- c("purple", "green", "salmon", "cyan")
...
gl.diversity.edited <- function (gl, spectrumplot = TRUE, confiplot = FALSE, probar = TRUE, 
  ...
  if (spectrumplot) {
    ...
    bb <- barplot(fs, beside = T, names.arg = rep(rownames(fs), 3), 
                  ylim = c(1, 2.15), 
                  main = "q-profile", 
                  ylab = "Equivalent number of equally frequent alleles (D)",
                  col = popcol, las = 2, xpd = FALSE, cex.names = cx)                        # <<< changed here
    text(colMeans(bb), rep(2.1, 5), labels = c("Allelic Richness",                           # 3>5 test
                                               "Shannon Index", "Heterozygosity"))            # <<< changed here
    sds <- cbind(zero_D_alpha_sd, one_D_alpha_sd, two_D_alpha_sd)
    ses <-  sds / sqrt(gl$n.loc)                                                              # added SE instead of STDV here
    up <- fs + ses
    low <- fs - ses
    if (confiplot) 
      for (i in 1:ncol(bb)) {
